...
if(isset($_GET['id'])){
    ...
    $apt = mysqli_query($conn, "SELECT * FROM appointment WHERE Doctor_ID='$id'");

    $New = mysqli_query($conn, "SELECT COUNT(Appointment_ID) AS New_Count FROM appointment WHERE Doctor_ID = '$id' and Status = 'New'");
    $row1 = mysqli_fetch_array($New,MYSQLI_ASSOC);
    $NC = $row1['New_Count'];

    $Posted = mysqli_query($conn, "SELECT COUNT(Appointment_ID) AS Posted_Count FROM appointment WHERE Doctor_ID = '$id' and Status = 'Posted'");
    $row2 = mysqli_fetch_array($Posted,MYSQLI_ASSOC);
    $PC = $row2['Posted_Count'];

    $query = "SELECT * FROM doctors WHERE Doc_ID = '$id'";
...
<body>
    <div id="container">
        ...
        <div id="content3">
            <div id="box3">
                <div id="b3">
                  <img src="uploads/<?php echo $img?>" alt="" id="img">
                  <form action="doctor_pic.php?id=<?php echo $Docid?>"
                        method="POST"
                        enctype="multipart/form-data"
                        style="display:flex; flex-direction:row;">
                        <input type="file" name="image" id="buttons">
                        <input type="submit" name="upload" id="buttons" value="Change Picture">
                    </form>
                  </div>
                <div id="b4">
                    <h1><b>DOCTOR ID: </b><?php echo $Docid?></h1>
                    ...
                    <a href="disable.php?id=<?php echo $id?>"><b>Disable Doctor</b></a>
                </div>
            </div>
            <div id="box4">
                <div id="c3">
