...
?>
<?php
$query = "SELECT * from doctors WHERE Status = 'Inactive' ";
$quer_run = mysqli_query($conn,$query);

$New = mysqli_query($conn, "SELECT COUNT(Doc_ID) AS Male_Count FROM doctors WHERE Gender = 'Male'");
$row1 = mysqli_fetch_array($New,MYSQLI_ASSOC);
$MC = $row1['Male_Count'];

$New1 = mysqli_query($conn, "SELECT COUNT(Doc_ID) AS Female_Count FROM doctors WHERE Gender = 'Female'");
$row2 = mysqli_fetch_array($New1,MYSQLI_ASSOC);
$FC = $row2['Female_Count'];

$New2 = mysqli_query($conn, "SELECT COUNT(Doc_ID) AS Active_Count FROM doctors WHERE Status = 'Active'");
$row3 = mysqli_fetch_array($New2,MYSQLI_ASSOC);
$AC = $row3['Active_Count'];

$New3 = mysqli_query($conn, "SELECT COUNT(Doc_ID) AS Inactive_Count FROM doctors WHERE Status = 'Inactive'");
$row4 = mysqli_fetch_array($New3,MYSQLI_ASSOC);
$IC = $row4['Inactive_Count'];
...
<body>
    <div id="container">
                      ...
                      <li><div id="dropdown">
                        ...
                        <div id="dropdown-content">
                          ...
                          if($Pos=='Doctor'){
                            ...
                            echo '<a href="#" style="color: white;">EDIT DOCTORS</a>';
                          }
                          ?>
                        </div>
                      </div></li>
        ...
        <div id="bod">
            ...
            <H1>LIST OF INACTIVE DOCTORS</H1>
            <div id="status-btn">
            <a href="seedoctors.php"style="text-decoration:none; font-weight:bold;font-size:20px;color:#333;">Active</a>
            <a href="#"style="text-decoration:none; font-weight:bold;font-size:20px;color:#333;">Inactive</a>
          </div>
                  ...
                  <table id="example" class="display table table-bordered data-table" style="width:100%">
                    ...
                    <?php
                            while($row = $quer_run->fetch_assoc()) {
                              echo "<tr>";
                              echo "<td> <img src='uploads/".$row['img_id']."' alt='' style='height:50px; width:50px; border-radius:100%; border-style:solid; border-width: 1px;'> </td>";
                              echo "<td>".$row['Name']."</td>";
                              echo "<td>".$row['Age']."</td>";
                              echo "<td>".$row['Spec']."</td>";
                              echo "<td>".$row['Service']."</td>";
                              echo "<td>".$row['Status']."</td>";
                              echo '<td> <a href="doctorprofile.php?id='.$row['Doc_ID'].'" style="text-decoration:none; color:#333; font-weight:bold;">View Profile</a> </td>';
                              echo "</tr>";
                            }
                            ?>
                    </tbody>
